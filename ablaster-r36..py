...
# for initial efforts Banana Gun bullets will be red, and will have zero mass
...
# constant for speed of pewPew bullets
        LAUNCHVELOCITY = 18  # this is the speed in excess of ship, travel slower than pewPew bullets
        self.timeToLive = 120  # frames before disappearing
...
# define the starting image (a small round cannonball)
        ...
        self.image.fill((0xff, 0xff, 0xfe))   # almost black
        self.image.set_colorkey((0xff, 0xff, 0xfe)) # set transparency
        pygame.draw.circle(self.image, (0xff,0xff,0), (5,5), 5) # some color
        self.rect = self.image.get_rect()
...
# make a "fired" sound
        ...
        if SOUND == True:
            if not pygame.mixer:
                ...
                pygame.mixer.init()
                self.sndFired = pygame.mixer.Sound("bananaFired.ogg")
                self.sndFired.play()    # blast away!
...
#Debug
                ...
                print "Ship %d: Current Weapon: %s, Remaining shots: %d" % (recipient.shipNumber, recipient.currentWeaponName, recipient.remainingShots)
...
# the rack and select the next weapon (if any).  Display "no weapon" or similar if the rack is empty
        ...
        self.weaponsrack = [("banana gun", 14), ("pew-pew gun", 288),("banana gun", 8) ]
